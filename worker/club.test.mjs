...
{
  const env = storage();
  assert.deepEqual((await call(env, '/club/health')).data, { club: false, mode: 'off', version: 1, batch: true, late_marks: true, forgiving_key: true, rejoin: true, remove: true, returning: true, passkeys: true, votes: true, storage: useD1 ? 'd1' : 'kv' });
