...
С 14.09.2026 приёмник — свой сервер в Москве, потому что телефоны в России без VPN не достают до Cloudflare: [club-server.md](club-server.md). Ниже — вариант на Cloudflare, с которого всё начиналось.
...
- В расчёте, спасибо и 👍/👎 отметка живёт три часа: устаревшее наблюдение не должно выдавать себя за текущее. Сервер хранит отметки сутки, и вкладка «👁 Свои» показывает их все (с 16.09.2026). Свежие там сверху, остальные открываются кнопкой и подписаны временем.
